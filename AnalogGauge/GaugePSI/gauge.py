...
def calibrate_gauge(gauge_number, file_type):
    ...
    # cv2.line(img, (x,0), (x,y),(0, 255, 0), 2)

    cv2.imwrite('img/gauge-%s-calibration.%s' % (gauge_number, file_type), img)
    ...
    return x, y, r # min_angle, max_angle, min_value, max_value, units, 
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    cv2.imwrite('img/gauge-%s-tempdst2.%s' % (gauge_number, file_type), dst2)

    # find lines
    minLineLength = 10
    maxLineGap = 0
    lines = cv2.HoughLinesP(image=dst2, rho=3, theta=np.pi / 180, threshold=100,minLineLength=minLineLength, maxLineGap=3)  # rho is set to 3 to detect more lines, easier to get more then filter them out later
    final_line_list = []
    ...
    cv2.putText(img, str(int(new_value))+"km/h", (x,y-50), cv2.FONT_HERSHEY_SIMPLEX, 1.5,(255,0,0),2,cv2.LINE_AA)
    cv2.imwrite('img/gauge-%s-Result.%s' % (gauge_number, file_type), img)
    cv2.imshow("img",img)
    cv2.waitKey(0)
